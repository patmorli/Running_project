...
data_name = "8k_250_0"
subjects = [1,2,3,8,9,20,21,24,28,32,33,35]
...
def parse_tfrecord_spectrogram(element):
  ...
  subject_id = tf.one_hot(subject_id, depth = 188, dtype = 'int64')
  speed = content['speed_label']
  speed = tf.one_hot(speed, depth = 4, dtype = 'int64')
  
  
  #get our 'feature'-- our image -- and reshape it appropriately
  feature = tf.io.parse_tensor(spectrogram_image, out_type=tf.double)
  feature = tf.reshape(feature, shape=[height,width,depth])
  return (feature, subject_id)




tense = list()
for batch in tf.data.TFRecordDataset(train_filenames).map(parse_tfrecord_spectrogram):
    #print(batch)
    tense.append(batch)
    #break
...
if flag_plot:
    plt.figure()
    plt.imshow(tense[1][0][:,:,4], cmap='hot', interpolation='nearest')
    plt.show()
...
def get_dataset(filenames, batch_size):
  ...
  dataset = (
          ...
          .map(parse_tfrecord_spectrogram)
