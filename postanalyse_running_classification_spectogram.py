...
model_name = 'quick_try_colab_frozen_nodrop'
flag_plot_loss = 1
flag_plot_model = 0
...
with open(dir_results_history + model_name + '.pkl', 'rb') as file_pi:
    [history, evaluated_train_loss, evaluated_val_loss, val_filenames, subjects, true_list_val, pred_list_val] = pickle.load(file_pi)  
...
if classification:
    if flag_plot_datapoints:
        ...
        val_dataset_true, val_dataset_pred_ = fcg.get_predictions_true_manually(val_dataset, my_model, steps_to_take)
        val_dataset_pred = np.argmax(val_dataset_pred_, axis=1)
        val_dataset_true = np.argmax(val_dataset_true, axis=1)
        f1 = f1_score(val_dataset_true, val_dataset_pred)
        
        # get same dataset but with seconds
        val_dataset_seconds = fcg.get_dataset_cont_unshuffled(val_filenames, batch_size)
        val_dataset_seconds_true, val_dataset_seconds_pred = fcg.get_predictions_true_manually(val_dataset_seconds, my_model, steps_to_take)
        ...
        for datapoint in range(len(val_dataset_seconds_true)):
            if val_dataset_pred[datapoint] == val_dataset_true[datapoint]:
    ...
    if flag_plot_confusion_matrix:
        cm = confusion_matrix(y_true=val_dataset_true, y_pred=val_dataset_pred)
