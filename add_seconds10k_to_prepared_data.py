...
How it works:
- This script adds the 10k time to each participant's running file'
"""
    ...
    for column in overview_columns:
        ...
        if type(key) == str:
            try:
                ...
                try:  
                    ## add score to data
                    seconds_10k = np.zeros(len(data)) + float(score_file.loc[i, ['seconds_10k']])
                    
                    data.insert(18,"seconds_10k", seconds_10k)
                    ...
                    print(key + ' already has a seconds_10k field bro')
            except:
